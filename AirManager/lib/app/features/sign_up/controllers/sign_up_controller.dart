...
class SignUpController extends GetxController with ValidatorMixin {
  ...
  void signUp() async {
    ...
    String? passwordConfirmError = isValidPassword(passwordConfirm.text);

    if (emailError != null) {
      AppSnackbar.showMessage(emailError);
    } else if (passwordError != null && passwordConfirmError != null) {
      AppSnackbar.showMessage(passwordError);
    } else {
      isLoading.value = true;
      try {
        if(password.text!=passwordConfirm.text){
          throw Exception("PASSWORT FALSCH");
        }
        ...
        AppSnackbar.showMessage("Wrong confirmation password!");
      }
